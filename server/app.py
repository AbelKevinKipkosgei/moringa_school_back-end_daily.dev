...
class SignupResource(Resource):
    def post(self):
        ...
        new_user = User(
            username=username, email=email, bio=bio, profile_pic_url=image_url
        )
...
class LogoutResource(Resource):
    ...
    def post(self):
        ...
        redis_client.setex(
            jti, int(app.config["JWT_ACCESS_TOKEN_EXPIRES"].total_seconds()), "true"
        )
    ...
    def get(self):
        ...
        for post in posts:
            ...
            serialized_post["is_wishlist_by_user"] = (
                post.is_wishlist_by_user(user_id) if user_id else False
            )
...
class CreatePost(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            "post_type", required=True, choices=["video", "audio", "blog"]
        )
        data = parser.parse_args()
        ...
        try:
            upload_result = cloudinary.uploader.upload(
                ...
                folder="thumbnails",
                transformation=[
                    {"width": 300, "height": 300, "crop": "thumb", "radius": 20}
                ],
            )
...
class ReadPost(Resource):
    def get(self, post_id):
        ...
        if not post:
            return {"message": "Post not found"}, 404
        return {
            "post": post.serialize_with_comments(page=1, per_page=10, max_depth=3)
        }, 200
...
class CategoryResource(Resource):
    ...
    def post(self):
        ...
        parser.add_argument(
            "description", required=True, help="Category description is required"
        )
        data = parser.parse_args()
...
class FlagPost(Resource):
    ...
    def post(self, post_id):
        ...
        parser = reqparse.RequestParser()
        parser.add_argument(
            "reason", required=True, help="Reason for flagging is required"
        )
        data = parser.parse_args()
...
class EditPost(Resource):
    ...
    def put(self, post_id):
        ...
        if not user:
            return {"messsage": "unauthorized user"}
...
api.add_resource(
    AdminReactivateUserResource,
    "/api/admin/user/reactivate/<int:user_id>",
    endpoint="reactivateuser",
)
api.add_resource(
    AdminDeactivateUserResource,
    "/api/admin/user/deactivate/<int:user_id>",
    endpoint="deactivateuser",
)
api.add_resource(
    AdminDeletePostResource,
    "/api/admin/post/delete/<int:post_id>",
    endpoint="deletepost",
)
...
api.add_resource(
    CategoryResource, "/api/categories/createcategory", endpoint="createcategory"
)
...
api.add_resource(
    SubscribeCategory,
    "/api/category/subscribe/<int:category_id>",
    endpoint="subscribecategory",
)
api.add_resource(
    UnsubscribeCategory,
    "/api/category/unsubscribe/<int:category_id>",
    endpoint="unsubscribecategory",
)
api.add_resource(
    AddToWishlist, "/api/post/addtowishlist/<int:post_id>", endpoint="addtowishlist"
)
api.add_resource(
    RemoveFromWishlist,
    "/api/post/removefromwishlist/<int:post_id>",
